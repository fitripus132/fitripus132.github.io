class WL3Game extends Phaser.Scene {
    constructor() {
        super('WL3Game')
    }
    create() {
        if(gameInfo.type == 'development') console.log('WL3Game created successfully')
        ...
        this.add.bitmapText(51, 242, 'arial', gameInfo.currentQuest+1 + '/' + gameJson.option.length, 20).setTint('0x000000').setOrigin(0.5) //-----revisi
        //quest
        this.add.image(863, 258, 'gamePack', 'g' + (gameInfo.game+1) + 'lv' + (gameInfo.level+1) + '_quest_' + formatWithZero(gameInfo.currentQuest+1,4) + '.png')
        //translate
        this.translate = this.add.image(153, 251, 'gamePack', 'g' + (gameInfo.game+1) + 'lv' + (gameInfo.level+1) + '_translate_' + formatWithZero(gameInfo.currentQuest+1,4) + '.png').setOrigin(0,0.5).setTint(0x000000)
        // console.log(this)
        //drop image
        this.dropX = 633
        this.dropY = 434
        ...
        this.numDrag = gameJson.option[gameInfo.currentQuest].length
        var offsetX = 256
        this.initPos = []
        ...
        for (var i = 0; i < this.numDrag; i++) {
            var drag = new WL3Drag(this, this.initPos[i].x, this.initPos[i].y, i)
    ...
    next() {
        ...
        if (gameInfo.currentQuest < gameJson.option.length) {
...
class WL3Drag extends Phaser.GameObjects.Container {
    constructor(scene, x, y, value) {
        super(scene, x, y)
        if(gameInfo.type == 'development') console.log('WL3Drag created successfully')
        this.value = value
        this.lock = false
        var size = {w: 210, h: 83}
        ...
        this.word = scene.add.image(0, 0, 'WLTexture', 'drag_' + formatWithZero(gameJson.option[gameInfo.currentQuest][value], 4) + '.png').setTint(0x000000)
        this.add(this.word)
